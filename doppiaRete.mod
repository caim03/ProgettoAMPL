...
z[1] * sum{j in 1..nl1}v1[j] * (-1 + 1/(1+exp(-(sum{k in 1..ingr1}win1[k,j]*xtr1[p,k]-win1[ingr1+1,j]))))
+
z[2] * sum{j in 1..nl2}v2[j] * (-1 + 1/(1+exp(-(sum{k in 1..ingr2}win2[k,j]*xtr2[p,k]-win2[ingr2+1,j]))))
-ytr[p])^2
...
z[1] * sum{j in 1..nl1}v1[j] * (-1 + 1/(1+exp(-(sum{k in 1..ingr1}win1[k,j]*xv1[p,k]-win1[ingr1+1,j]))))
+
z[2] * sum{j in 1..nl2}v2[j] * (-1 + 1/(1+exp(-(sum{k in 1..ingr2}win2[k,j]*xv2[p,k]-win2[ingr2+1,j]))))
-yv[p]);
